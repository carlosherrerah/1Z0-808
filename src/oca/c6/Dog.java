...
public class Dog {
  ...
  static List<Dog> edadMinima(List<Dog> dogList, Integer edadTest) {
    ...
    for (Dog dog : dogList) {
      if (dog.edad >= edadTest) {  // query expresion
        result.add(dog);
      }
    }
    return result;
  ...
  static List<Dog> pesoMaximo(List<Dog> dogList, Integer pesoTest) {
    List<Dog> result = new ArrayList<>();

    for (Dog dog : dogList) {
      if (dog.peso <= pesoTest) { // query expresion
        result.add(dog);
      }
    }
    return result;
  ...
  static List<Dog> dogQuery(List<Dog> dogList, Predicate<Dog> expr) { // Predicate
    List<Dog> result = new ArrayList<>();
