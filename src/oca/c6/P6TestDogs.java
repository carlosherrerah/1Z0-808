...
public class P6TestDogs {
    ...
    public static void main(String[] args) {
        ...
        result = dogs.stream().filter(d -> d.nacimiento.isAfter(LocalDate.of(2017, 12, 31)))
                .collect(Collectors.toList());
        System.out.println("\nMayores al 2017-: " + result);

        // Perros con peso creciente ---------------------------------------------------------------------
        List<Dog> resultado = new ArrayList<>();
        dogs.stream()
                .filter(dog -> {
                    if (resultado.isEmpty() || dog.peso > resultado.get(resultado.size() - 1).peso) {
                        resultado.add(dog);
                        return true;
                    }
                    return false;
                })
                .collect(Collectors.toList()); // opcional, el efecto está en result

        System.out.println("\nPerros con peso creciente con stream:");
        ...
        System.out.println("\nPerros con peso creciente con forEach:");
        resultado.forEach(System.out::println);
    }
